...
class SqliteProductRepository(ProductRepository):
    ...
    # ---------- queries (admin) ----------
    def list_products_summary(self, search: Optional[str] = None) -> list[dict]:
        sql = """
        WITH latest AS (
    ...
    def get_minimum_price_history(self, product_id: UUID) -> list[dict]:
        """Returns the minimum price for the product on each date it was fetched."""
        sql = """
        SELECT DATE(ps.fetched_at) AS date, MIN(ps.price) AS min_price
        FROM listings l
        JOIN price_snapshots ps ON ps.listing_id = l.id
        WHERE l.product_id = ?
          AND l.link_status IN ('auto', 'confirmed')
        ...
        with self._conn() as conn:
            return [
                {"x": r["date"], "y": r["min_price"]}
                for r in conn.execute(sql, (str(product_id),)).fetchall()
            ]

    # ---------- queries (public) ----------
    def list_products_public_view(self, search: Optional[str] = None) -> list[dict]:
